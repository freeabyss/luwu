...
- 多文档输出到 `docs/prd/` 目录：
  - `README.md`：主文档（产品概述、用户分析、产品设计、功能需求索引、权限、指标、发布、验收）
  - `glossary.md`：术语表（中英文、含义、计算公式）
  - `features/Fxx-*.md`：**每个功能一个子文档**（功能描述、用户流程、页面设计、业务规则、数据定义、异常处理）
...
- 重构 PRD 模板：主文档 `README.md` 采用「产品概述 / 用户分析 / 产品设计 / 功能需求 / 权限设计 / 数据分析指标 / 发布计划 / 验收标准」固定大纲
- 术语表独立为 `glossary.md`（术语中英文、含义、计算公式），主文档 3.3 只保留入口链接
- 功能需求由单文件 `features.md` 改为**每个功能一个子文档** `features/Fxx-{名称}.md`，统一包含功能描述、用户流程、页面设计、业务规则、数据定义、异常处理，末尾维护历史版本与变更记录
- `prototype.md` 收敛为 UI/UX 7 维度规范 + 全局页面导航，逐页线框图下沉到各功能子文档的「4.3 页面设计」
- 同步更新 prd skill 工作流（维护规则、起草步骤、自审清单、返回值）、evals 预期输出与文档说明
