...
**项目文档目录自动探测**（不硬编码 `doc/`）：用户指定 > `docs/README.md` > `doc/README.md` > 已有 prd 所在目录。**必须先读文档根下的 `README.md`**，按其指引定位目录。
...
1. **探索项目上下文**：
   ...
   - Glob 查找已有 PRD：查找 `docs/prd/`、`doc/prd/`、`docs/prd.md` 等
...
3. 将结论写入 `ui-ux.md` 子文档。
4. 若调用失败或结论不明确，在主文档「待解决问题」中标注「UI/UX 设计规范未完成」，列出具体待决项，PRD 其他部分继续输出（不阻塞）。
...
2. 将结论（图表类型选择、配色、交互、可访问性）融入对应功能描述或 `ui-ux.md`。
...
| `ui-ux.md` | UI/UX 设计规范（7 维度结论） | Phase 3.1 触发时创建 |
...
6. 将 UI/UX 7 维度结论写入 `ui-ux.md`，数据可视化结论融入对应位置。
