...
2. **功能级变更记录**：某功能发生新增、修改、删除、废弃、规则调整、页面调整、字段调整时，在该功能子文档 `features/Fxx-{名称}.md` 末尾维护：
  - `## 历史版本`（版本、日期、变更类型、摘要、影响范围、作者）
  - `## 变更记录`（日期、变更前、变更后、原因、关联来源）
  - 新增功能时新建 `features/Fxx-{名称}.md`，并在主文档功能清单（3.2.2）和功能需求索引（第 4 章）中登记。
3. **文档级变更记录**：PRD 结构、全局目标、发版计划、权限设计、数据分析指标、依赖系统、风险管理、非功能需求等全局内容变更，更新主文档 `README.md` 的"文档记录"。
4. **术语变更**：新增/修改业务术语时更新 `glossary.md`，保持全 PRD 用词一致。
5. **正文同步更新**：历史记录不替代正文，变更后功能正文、流程、页面交互、处理逻辑、业务规则等当前有效内容必须同步更新。
6. **子文档变更**：若变更仅涉及某子文档主题（如只改 UI/UX），只更新对应子文档，主文档索引表更新"最后更新"日期。
...
3. 基于上述设计规范，梳理页面原型：
  - 在 `prototype.md` 中列出全局页面/视图清单、导航关系与页面流程图（Mermaid）
  - 各功能特有的页面布局线框图、核心区域、关键组件、交互行为、页面状态，写入对应 `features/Fxx-{名称}.md` 的「4.3 页面设计」
4. 将 UI/UX 设计规范（7 维度）+ 全局页面清单/导航统一写入 `prototype.md` 子文档；逐页线框图下沉到功能子文档。
...
2. 将结论（图表类型选择、配色、交互、可访问性）写入对应功能子文档的「4.3 页面设计」或 `prototype.md` 的全局规范中。
...
- **主文档 `README.md`** 按固定大纲组织：产品概述、用户分析、产品设计（含功能清单）、功能需求索引、权限设计、数据分析指标、发布计划、验收标准。
- **术语表独立为 `glossary.md`**：业务术语、含义、计算公式、中英文名称统一维护于此，主文档 3.3 链接过去。
- **每个功能需求独立成一个子文档 `features/Fxx-{名称}.md`**：统一包含功能描述、用户流程、页面设计、业务规则、数据定义、异常处理，末尾维护历史版本与变更记录。不允许把多个功能塞进同一个文件。

| 文档                  | 内容                                                  | 何时创建            |
| ------------------- | --------------------------------------------------- | --------------- |
| `README.md`（主文档）    | 文档信息、产品概述、用户分析、产品设计（架构/模块/功能清单/术语入口）、功能需求索引、权限设计、数据分析指标、发布计划、验收标准、待解决问题、文档索引表 | **必建**          |
| `glossary.md`       | 术语表（术语、英文、含义、计算公式）                                  | 存在业务术语时创建       |
| `features/Fxx-*.md` | 每个功能一个子文档（功能描述、用户流程、页面设计、业务规则、数据定义、异常处理、历史版本、变更记录） | **必建，至少一个**    |
| `user-stories.md`   | 用户故事、角色定义、用例描述、详细流程时序图                              | 用户角色复杂或流程复杂时创建  |
| `prototype.md`      | UI/UX 设计规范（7 维度结论）+ 全局页面清单/导航/流程；逐页线框图下沉到功能子文档     | Phase 3.1 触发时创建 |
...
| features/F01-登录.md | F01 登录功能需求 | YYYY-MM-DD |
| prototype.md | UI/UX 设计规范 + 页面导航 | YYYY-MM-DD |
```

**小型需求**（单个功能点、Bug 修复、简单变更）仍需 `README.md` + `features/Fxx-*.md`；无业务术语时可省略 `glossary.md`，不拆其他子文档。
...
2. 先创建主文档 `README.md` 骨架和索引表，再创建 `glossary.md`（如有术语），然后为**每个功能**创建独立的 `features/Fxx-{名称}.md`，最后按需创建其他子文档。
3. 功能需求必须可验证，不写"正常工作""优化体验"等含糊表述；业务规则、异常处理写在对应功能子文档中，主文档验收标准不重复功能细节。
4. 若涉及界面，在该功能子文档「4.3 页面设计」中描述页面交互、校验、异常提示；全局设计规范放 `prototype.md`。
5. 若涉及系统实现，写明依赖系统、接口、数据、权限、状态流转、异常处理；功能特有数据放子文档「4.5 数据定义」，全局数据模型放 `data-model.md`。
6. 将 UI/UX 7 维度结论 + 全局页面清单/导航写入 `prototype.md`，数据可视化结论融入对应位置。
7. 若信息缺失，不要编造关键事实；在主文档「待解决问题」中标注。
8. 使用 Write 工具输出。已有 PRD 存在时，对已有文件使用 Edit 修改，不新建重复文件；新增功能才新建 `features/Fxx-*.md`。
9. 每个功能子文档末尾维护历史版本与变更记录（追加式，不覆盖历史）；主文档维护「文档记录」。
...
- 各子文档之间是否有矛盾？主文档功能清单（3.2.2）与功能需求索引（第 4 章）的编号是否与 `features/Fxx-*.md` 文件一一对应？
- 每个功能是否都有独立子文档？是否误把多个功能塞进同一文件？
...
> - 功能详情：docs/prd/features/Fxx-*.md（每个功能一个文件）
> - 术语表：docs/prd/glossary.md（若创建）
> - {其他创建的子文档列表}
>
...
产物路径：
- 主文档：docs/prd/README.md
- 功能子文档：docs/prd/features/F01-xxx.md, docs/prd/features/F02-xxx.md
- 术语表：docs/prd/glossary.md（若有）
- {其他子文档}
任务类型：新建/维护/改写
结论：待用户 review / 已确认
摘要（≤5行）：
- 功能清单：F01 xxx, F02 xxx（每功能独立子文档）
- 子文档：glossary.md, prototype.md
- 待解决问题：N 项
```
